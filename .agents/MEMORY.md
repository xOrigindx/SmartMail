...
- **GIT PUSH COMMANDS**: The project uses two repositories. 
  - When the user says "push", push ONLY to the dev repo (`developpement`) from the `main` branch.
  - When the user says "push public", push ONLY to the public repo (`public`). Do this by switching to the `public-release` branch, merging updates from `main`, THEN IMMEDIATELY running `git rm -rf --ignore-unmatch .agents/ Scripts/ Docs/ TODO_STRESS.lua AGENTS.md public_repo.gitignore` to strip all development files, committing, and running `git push public public-release:main`. Do NOT swap `.gitignore` files on `main`.
